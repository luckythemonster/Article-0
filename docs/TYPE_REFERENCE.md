...
`src/systems/EntityStats.ts:750`

| Key | Value | Notes |
| --- | --- | --- |
| `interactionTime` | `1.4` |  |
| `noiseOnOpen` | `3` |  |
| `items` | `["Medkit", "Battery"]` | Last-resort loot, for a chest carrying no loot field of any kind. **This used to be what every chest on the shipped map yielded**, because the engine read `item1/2/3` and the map authors `items` — see `chestLoot`. Now that both schemas are read it is genuinely a fallback, and on the shipped map it is unreachable: the tile editor substitutes the `Chest` DataStructure's own `DefaultValues` for any field an author leaves null, so even the two chests that look blank (`main2vault`, `secret2`) arrive carrying a value. That schema default is `"Medkit", "Battery` — with the closing quote genuinely missing in `edplay.json`. `parseItemList` is tolerant of it deliberately rather than by accident: those two chests are the only thing that depends on stray quotes being stripped rather than being taken literally. Ordered so that since unlit space became genuinely opaque a Battery outranks Stun Rounds — light is load-bearing, stunning an Orderly bystander is a convenience. **Two entries, not three, since the Access Chit became a keycard.** The slot schema has three and the third was the chit, which opened nothing; a keycard is not a drop-in replacement for it, because a *numbered* credential has to match a door somebody authored and a default table cannot know which. Seeding one here would put the same clearance in every unauthored chest on every map — placement dressed up as a default. A chest that wants to hand out a keycard should say which. |

<a id="const-consumable-order"></a>

#### `CONSUMABLE_ORDER` — const

`src/systems/EntityStats.ts:1450`
...
`src/systems/EntityStats.ts:545`
...
`src/systems/EntityStats.ts:620`
...
<a id="class-firearmsauthorization"></a>

#### `FirearmsAuthorization` — class

`src/systems/Firearms.ts:34`

The base-wide release that a firearm needs before it will go off.

**Two gates, not one.** A shot requires an `./EntityStats.EnforcerStats``.armed`
...
authorization accrues across `FIREARMS_AUTHORIZATION_DELAY` seconds of
...
| Member | Signature | Notes |
| --- | --- | --- |
| `posture` | `get posture(): FirearmsPosture` | The posture, for the HUD and the debug overlay. |
| `authorized` | `get authorized(): boolean` | True once weapons are free. What `Enforcer.pursue` reads. |
| `secondsToAuthorization` | `get secondsToAuthorization(): number` | Seconds of sustained ALERT still owed before weapons are released — 0 once they are. Exposed for the debug overlay, which is where this is legible at all: the mechanic is otherwise invisible right up until the moment it isn't. |
| `update` | `update(dt: number, phase: AlertPhase): void` |  |
...
`src/systems/EntityStats.ts:741`
...
`src/systems/EntityStats.ts:1497`
...
`src/systems/EntityStats.ts:536`
...
| `armed` | `boolean` | Whether this guard carries a firearm at all. **False is the default, and the default is the restriction.** A gun needs two gates to go off: this flag, and a base-wide release (`./Firearms.FirearmsAuthorization`). Neither alone is enough, which is what keeps a firearm an escalation the building reaches for rather than the thing every body on the roster is holding. The per-level complement is capped at `ARMED_POSTS_PER_LEVEL` where the cast is spawned, so setting this true on a map board cannot flood a level. |
| `fireRange` | `number` | Reach of the pursuing-guard ranged attack, in tiles. Only fires when `armed`. |
| `fireCooldown` | `number` | Seconds between shots. |
| `fireDamage` | `number` | Bio-integrity damage per shot that reaches the player. |
| `meleeRange` | `number` | Reach of the pursuing-guard contact attack, in tiles — the default answer. |
| `meleeCooldown` | `number` | Seconds between strikes. |
| `meleeDamage` | `number` | Bio-integrity damage per strike. |
...
`src/systems/EntityStats.ts:594`
...
`src/systems/EntityStats.ts:113`

| Field | Type | Notes |
| --- | --- | --- |
| `radius` | `number` |  |
| `detectionMultiplier` | `number` |  |
| `brightness` | `number` | How bright the pool is, 0-1. How *far* it reaches is `radius`, and the two are genuinely different questions — a fixture can light the same room and light it less. Added for emergency lighting, which needs to read as dim rather than as small: a lamp shrunk until it looked dim would stop reaching the doorway, and one left at full strength was measurably indistinguishable from the overhead it replaced (identical lit area, mean brightness 0.376 against 0.381). Only the *visible* half: how easily a guard sees you in a pool is `detectionMultiplier`'s job, and a lamp can be dim without being safe. |
...
`src/systems/EntityStats.ts:710`
...
`src/systems/EntityStats.ts:832`
...
| Field | Type | Notes |
| --- | --- | --- |
| `circuits` | `Record<string, boolean>` | `circuitKey` -> whether the circuit is closed (powered). Strictly **the position of the control named by the key**: a breaker's lever at its wing, a wall switch's rocker at its zone. Not "is this zone lit" — a zone whose plate is on can still be dark because the wing above it is out, and telling those two apart is the whole of `hacked` and of the emergency lighting that hangs off it. See `src/scenes/game/PowerControl.ts`. |
| `hacked` | `Record<string, true>` | `circuitKey` -> zones a terminal hack cut, upstream of any plate. Separate from `circuits` because a hack has no fixture and therefore no lever to record a position for. Recording it as one would make a hacked room indistinguishable from a room somebody switched off by hand — and those two differ in what the player sees (a dead plate versus a lit `OFF` one) and in whether the emergency light comes up. |
...
`src/systems/EntityStats.ts:1725`
...
| `beamGiveawayPx` *(opt)* | `number` | How far the player's lit flashlight gives their position away, in px. `0` — or absent — when the beam is off, which is also every eye on a level nobody carries a torch on. One number rather than a flag and a range, so the "is it on" test and the "how far" test are the same comparison. Exactly the shape `Eye.thermalTiles` already uses, and read the same way. |

<a id="interface-sensorstats"></a>

#### `SensorStats` — interface

`src/systems/EntityStats.ts:632`
...
`src/systems/EntityStats.ts:1653`
...
`src/systems/EntityStats.ts:611`
...
`src/systems/EntityStats.ts:1529`
...
<a id="type-firearmsposture"></a>

#### `FirearmsPosture` — type

`src/systems/Firearms.ts:10`
...
```ts
type FirearmsPosture = "RESTRICTED" | "AUTHORIZED";
```
...
— see `opensWith`. A door authored `LOCKED` with **no** id is sealed
...
| `isManual` | `get isManual(): boolean` | Whether this door takes no credential at all. **Not the player's question — the *guards'*.** `GameScene.guardOperableDoorAt` reads this to decide what a patrol may work for itself, and a keycard door is a chokepoint for them too. Teaching it about inventory would hand every guard on the level whatever Rowan is carrying, so the player's path goes through `opensWith` instead and this stays a property of the door alone. |
| `opensWith` | `opensWith(inventory: readonly string[]): boolean` | Whether whoever is holding `inventory` can open this by hand. A keycard does not *unlock* the door — `locked` stays `readonly`, because the door has not changed. It says this opener is carrying the credential the door asks for, which is why the answer is a function of who is standing there rather than state anybody mutates. A `state: "locked"` door with `key: 0` names no clearance, so nothing opens it by hand however well equipped Rowan is; a terminal hack is the only way through. |
...
`src/entities/LightSwitch.ts:70`
...
| `isClosed` | `get isClosed(): boolean` | True when this plate is set to on. Says nothing about whether it has power. |
| `isLive` | `get isLive(): boolean` | True when power reaches this plate, so flipping it would do something. |
| `setLive` | `setLive(live: boolean): void` | Told by `PowerControl` whenever the circuit above this plate changes. |
...
| `stagger` | `stagger(seconds: number = GUARD_MELEE_STAGGER_SECONDS): void` | Slows Rowan for `GUARD_MELEE_STAGGER_SECONDS` — a guard's strike connecting. Separate from `takeDamage` and deliberately **not** behind its hit-cooldown: the two model different things. Damage is gated so a hazard cannot drain the bar in a frame; the stagger is the guard buying himself the ground he needs to keep hitting you, and refreshing it on a blow that landed is the correct behaviour. It is the whole reason a silicate's prod is dangerous — see `GUARD_MELEE_STAGGER_SECONDS` for why one of them cannot on its own hold Rowan inside the capture window that ends the run. |
| `staggered` | `get staggered(): boolean` | True while a guard's strike still has him off balance — read by the HUD. |
...
<a id="interface-enforcerattackresult"></a>
...
an `../systems/EntityStats.EnforcerStats``.armed` guard and a facility that
has released weapons (`../systems/Firearms.FirearmsAuthorization`) — so on a
...
| `beamGiveawayPx` *(opt)* | `number` | How far a lit flashlight betrays the player, in px — see `SensingWorld`. |
...
| `alert` | `AlertState` |  |
| `firearms` | `FirearmsAuthorization` | Whether the facility has released firearms this frame. Read only by `Enforcer.pursue`, and only for a guard that carries one at all. Base-wide rather than per-guard because that is what it models: a building deciding the situation warrants weapons, not a man deciding he has had enough. |
...
<a id="interface-armableguard"></a>

#### `ArmableGuard` — interface

`src/map/ArmedPosts.ts:5`
...
| `defaults` *(opt)* | `Record<string, string>` | The *editor's* own `DefaultValues` for this component's fields. Kept beside `values` rather than folded into it, because the two mean opposite things and folding them is what hid a map-wide bug. The editor fills every blank field with its structure's default, so by the time a component reaches here "the author chose 7" and "the author chose nothing" are the same string — and the editor's placeholder then silently outranked the engine's own tuned default. On the shipped map that took every light's detection multiplier to the editor's `1`, which is to say it removed the penalty for standing in the light from the entire game. `EntityStats.num` compares against this to tell blank from chosen. Optional because generated tiles and test fixtures build components by hand and have no schema behind them; absent means "every value here was chosen". |
...
`src/scenes/game/PowerControl.ts:99`
...
| `setCircuit` | `setCircuit(target: string, closed: boolean): void` | Moves one control's lever and re-applies everything it feeds. `target` is a *control's* name — a breaker's wing, a plate's zone, or an authored ref like `light_overhead1` — and `closed` is where that control now sits. What the zones under it actually do is worked out by `applyZone`, because a lever position is only half the answer. |
...
`src/scenes/game/SensingContext.ts:59`
...
`src/scenes/game/PowerControl.ts:64`
...
| `zoneWings` | `zoneWings(): ReadonlyMap<string, string>` | Every derived zone on the level, mapped to the wing above it. The reverse of `circuitsFor`, and needed for the half of the mechanic that reads *upward*: whether a zone has power at all depends on its wing's breaker, and a zone cannot find that out from its own name. The keys double as the level's zone list, which is what `PowerControl.restore` walks. Empty on a map with no derived lighting, which is what makes every target on such a map behave exactly as it did before any of this existed. |
| `lightSwitches` | `lightSwitches(): readonly LightSwitch[]` | The plates, so a dead circuit can grey the ones hanging off it. |
...
`src/scenes/game/SensingContext.ts:26`
...
| `detection` | `DetectionSystem` |  |
| `alert` | `AlertState` |  |
| `firearms` | `FirearmsAuthorization` | Whether the facility has released firearms — see `src/systems/Firearms.ts`. |
...
| `beamGiveawayTiles` | `number` | How far a lit beam betrays Rowan, in tiles — see `FLASHLIGHT_GIVEAWAY_TILES`. The other half of `flashlightMultiplier`, and the half that bites first: that one makes a guard who already has him fill faster, this one is what gets him noticed at all. |
...
`src/scenes/game/ItemActions.ts:487`
...
| `brightness` | `number` | How bright this fixture burns when powered, 0-1 — see `LightStats.brightness`. Distinct from `intensity`, which is this multiplied by whatever the flicker is doing this frame and zeroed outright when the circuit is dead. |
...
| [BreakerStats](#interface-breakerstats) | interface | `src/systems/EntityStats.ts:671` |
...
| [CHEST_DEFAULTS](#const-chest-defaults) | const | `src/systems/EntityStats.ts:750` |
| [ChestStats](#interface-cheststats) | interface | `src/systems/EntityStats.ts:741` |
...
| [CONSUMABLE_ORDER](#const-consumable-order) | const | `src/systems/EntityStats.ts:1450` |
| [ConsumableSlot](#interface-consumableslot) | interface | `src/systems/EntityStats.ts:1497` |
...
| [DetectionWorld](#interface-detectionworld) | interface | `src/systems/Sensing.ts:86` |
...
| [DOOR_DEFAULTS](#const-door-defaults) | const | `src/systems/EntityStats.ts:545` |
...
| [DoorStats](#interface-doorstats) | interface | `src/systems/EntityStats.ts:536` |
...
| [Enforcer](#class-enforcer) | class | `src/entities/Enforcer.ts:247` |
...
| [GlassStats](#interface-glassstats) | interface | `src/systems/EntityStats.ts:594` |
...
| [LightSwitchStats](#interface-lightswitchstats) | interface | `src/systems/EntityStats.ts:710` |
...
| [ParsedMap](#interface-parsedmap) | interface | `src/map/EdplayLoader.ts:271` |
...
| [PlayerStats](#interface-playerstats) | interface | `src/systems/EntityStats.ts:832` |
...
| [PowerWorld](#interface-powerworld) | interface | `src/scenes/game/PowerControl.ts:64` |
...
| [RelayStats](#interface-relaystats) | interface | `src/systems/EntityStats.ts:1725` |
...
| [SensorStats](#interface-sensorstats) | interface | `src/systems/EntityStats.ts:632` |
...
| [SmacStats](#interface-smacstats) | interface | `src/systems/EntityStats.ts:1653` |
...
| [TERMINAL_DEFAULTS](#const-terminal-defaults) | const | `src/systems/EntityStats.ts:620` |
| [TerminalHacks](#class-terminalhacks) | class | `src/scenes/game/TerminalHacks.ts:80` |
| [TerminalStats](#interface-terminalstats) | interface | `src/systems/EntityStats.ts:611` |
...
| [Vent4Stats](#interface-vent4stats) | interface | `src/systems/EntityStats.ts:1529` |
